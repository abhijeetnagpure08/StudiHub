...
courseRouter.get("/singleRoute/:courseId", async (req, res) => {
  const { courseId } = req.params;

  try {
    // Convert the courseId to ObjectId
    const courseIdAsObjectId = new mongoose.Types.ObjectId(courseId);

    const singleProductPage = await CourseModel.findOne({ _id: courseIdAsObjectId });
